...
import { Check, X, ArrowRight, Trophy, Zap, Loader2, Play, Sparkles } from "lucide-react"
...
interface MCQQuestion {
  id: string;
  sentence?: string; // used in grammar-puzzle, etc.
  sentenceBefore?: string; // used in article-alley, etc.
  sentenceAfter?: string; // used in article-alley, etc.
  word?: string; // used in spelling-bee, synonym-sprint, etc.
  words?: string[]; // used in odd-one-out
  paragraph?: string; // used in context-detective
  options: string[];
  correctIndex: number;
  explanation?: string;
  timeLimit?: number; // used in vocabulary-speedrun
  grammarPoint?: string; // used for SRS weakness tracking
  question?: string; // used in daily-workout
}

interface GenericMCQGameProps {
  modeId: string;
  title: string;
  description: string;
...
export function GenericMCQGame({ modeId, title, description, questionCount = 5 }: GenericMCQGameProps) {
  const {
    status,
    questions,
    currentIndex,
  ...
  const handleTimeUp = useCallback(() => {
    if (!showResult && currentQuestion) {
      ...
      setShowResult(true)
      submitAnswer(false, currentQuestion.grammarPoint)
      setStreak(0)
    }
  }, [showResult, currentQuestion, submitAnswer])
  ...
  if (status === 'loading') {
    return (
      <Card className="border-2 text-center p-12 flex flex-col items-center justify-center">
        ...
        <p className="text-muted-foreground">The AI is creating your personalized content...</p>
      </Card>
    );
  }
        ...
        <CardHeader className="pb-3 sm:pb-6">
          <div className="flex justify-between items-start">
            <CardTitle className="text-xl sm:text-2xl leading-relaxed flex-1">
              {currentQuestion.question && <span>{currentQuestion.question}</span>}
              {currentQuestion.sentence && <span>{currentQuestion.sentence}</span>}
              {currentQuestion.paragraph && <p className="text-lg font-medium">{currentQuestion.paragraph}</p>}
              {currentQuestion.word && <span className="text-3xl font-bold text-indigo-600 dark:text-indigo-400 block mb-2">{currentQuestion.word}</span>}
              {currentQuestion.words && (
                <div className="flex flex-wrap gap-2">
                  {currentQuestion.words.map((w, i) => <Badge key={i} variant="secondary" className="text-lg">{w}</Badge>)}
                </div>
              )}
              {(currentQuestion.sentenceBefore || currentQuestion.sentenceAfter) && (
                <span>
                  {currentQuestion.sentenceBefore}
                  <span className="inline-block min-w-[80px] mx-2 border-b-2 border-indigo-500 px-2 text-center text-indigo-500">
                    {showResult && selectedAnswer !== -1 ? currentQuestion.options[selectedAnswer!] : "..."}
                  </span>
                  {currentQuestion.sentenceAfter}
                </span>
              )}
            </CardTitle>
            {(currentQuestion.sentence || currentQuestion.word || currentQuestion.sentenceBefore) && (
              <AudioButton text={currentQuestion.sentence || currentQuestion.word || `${currentQuestion.sentenceBefore} blank ${currentQuestion.sentenceAfter}`} />
            )}
          </div>
        </CardHeader>
              ...
              >
                ...
                <div>
                  <p className={cn("font-semibold mb-1", isCorrectAnswer ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400")}>
                    {isCorrectAnswer ? "Correct!" : selectedAnswer === -1 ? "Time's up!" : "Not quite right"}
                  </p>
